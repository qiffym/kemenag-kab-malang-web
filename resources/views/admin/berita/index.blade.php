...
@push('kelola-berita-active', 'active')

@section('admin-layout')
<div class="row">
  <div class="col-md-12 fw-bold fs-3">Kelola Berita & Artikel</div>
  <hr class="mb-4">
</div>
<div class="row justify-content-between">
  <div class="col-4">
    <a name="" id="" class="btn btn-primary" href="/admin/berita/create" role="button"><i class="bi bi-plus"></i> Tulis
      Berita Baru</a>
  </div>
  <div class="col-4">
...
<div class="table-responsive-xxl">
  <table class="table table-striped table-hover">
    ...
    <thead class="text-center">
      <tr>
        <th scope="col">No.</th>
        <th scope="col" width="350">Judul Berita</th>
        <th scope="col" width="500">Content</th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ $post->title }}</td>
        <td>{!! $post->excerpt !!}</td>
        <td class="text-center align-middle"><img src="{{ asset('') }}/assets/img/post/{{ $post->image_path }}"
            alt="image" width="100">
        </td>
        {{-- Details --}}
        <td class="align-middle text-start text-xxl-center">
          <small>
            <i class="bi bi-bar-chart-line-fill me-3"> {{ $post->click }}</i> <br>
            <i class="bi bi-person-fill me-3">
              {{  $post->author->name  }}</i> <br>
            <i class="bi bi-calendar-date"> {{ \Carbon\Carbon::parse($post->publish_at)->isoFormat('D-MMM-Y') }}</i>
            <br>
        ...
        <td class="text-center">
          {{-- Tampilkan --}}
          <form action="/admin/berita/{{ $post->id }}/activate" method="post" class="d-inline-block">
            @csrf
            @method('POST')
            @if ($post->active == '1')
            <button type="submit" name="unshow" id="unshow" class="btn btn-outline-success">
              <i class="bi bi-eye-fill"></i></button>
            @else
            <button type="submit" name="show" id="show" class="btn btn-outline-danger">
          ...
          {{-- Update --}}
          <a name="" id="" class="btn btn-warning" href="/admin/berita/{{ $post->slug }}/edit" role="button">
...
<script>
  $(document).on('click','.delete',function(){
         ...
         $('#id').val(id);
         $('#delModalForm').attr('action','/admin/berita/' + id);
